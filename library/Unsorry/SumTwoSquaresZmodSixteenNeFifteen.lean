...
/-- Goal `sum-two-squares-zmod-sixteen-ne-fifteen`: over `ℤ`, `a ^ 2 + b ^ 2` is never `15` in `ZMod 16`,
by reducing the integer cast to `ZMod 16` and a finite case check. -/
theorem sum_two_squares_zmod_sixteen_ne_fifteen (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 16) ≠ 15 := by
