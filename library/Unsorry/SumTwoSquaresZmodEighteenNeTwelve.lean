...
/-- Goal `sum-two-squares-zmod-eighteen-ne-twelve`: over `ℤ`, `a ^ 2 + b ^ 2` is never `12` in `ZMod 18`,
by reducing the integer cast to `ZMod 18` and a finite case check. -/
theorem sum_two_squares_zmod_eighteen_ne_twelve (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 18) ≠ 12 := by
