...
/-- Goal `sum-two-squares-zmod-twenty-ne-eleven`: over `ℤ`, `a ^ 2 + b ^ 2` is never `11` in `ZMod 20`,
by reducing the integer cast to `ZMod 20` and a finite case check. -/
theorem sum_two_squares_zmod_twenty_ne_eleven (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 20) ≠ 11 := by
