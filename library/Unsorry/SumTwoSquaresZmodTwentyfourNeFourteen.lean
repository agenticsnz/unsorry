...
/-- Goal `sum-two-squares-zmod-twentyfour-ne-fourteen`: over `ℤ`, `a ^ 2 + b ^ 2` is never `14` in `ZMod 24`,
by reducing the integer cast to `ZMod 24` and a finite case check. -/
theorem sum_two_squares_zmod_twentyfour_ne_fourteen (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 24) ≠ 14 := by
