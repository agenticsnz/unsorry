...
/-- Goal `sum-two-squares-zmod-twenty-ne-three`: over `ℤ`, `a ^ 2 + b ^ 2` is never `3` in `ZMod 20`,
by reducing the integer cast to `ZMod 20` and a finite case check. -/
theorem sum_two_squares_zmod_twenty_ne_three (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 20) ≠ 3 := by
