...
/-- Goal `sum-two-cubes-zmod-twentyone-ne-ten`: over `ℤ`, `a ^ 3 + b ^ 3` is never `10` in `ZMod 21`,
by reducing the integer cast to `ZMod 21` and a finite case check. -/
theorem sum_two_cubes_zmod_twentyone_ne_ten (a b : ℤ) : (((a ^ 3 + b ^ 3 : ℤ)) : ZMod 21) ≠ 10 := by
