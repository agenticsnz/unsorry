...
/-- Goal `sum-two-cubes-zmod-fourteen-ne-three`: over `ℤ`, `a ^ 3 + b ^ 3` is never `3` in `ZMod 14`,
by reducing the integer cast to `ZMod 14` and a finite case check. -/
theorem sum_two_cubes_zmod_fourteen_ne_three (a b : ℤ) : (((a ^ 3 + b ^ 3 : ℤ)) : ZMod 14) ≠ 3 := by
