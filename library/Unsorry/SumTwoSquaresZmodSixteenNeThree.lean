...
/-- Goal `sum-two-squares-zmod-sixteen-ne-three`: over `ℤ`, `a^2 + b^2` is never `3` in `ZMod 16`,
by reducing the integer cast to `ZMod 16` and a finite case check. -/
theorem sum_two_squares_zmod_sixteen_ne_three (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 16) ≠ 3 := by
