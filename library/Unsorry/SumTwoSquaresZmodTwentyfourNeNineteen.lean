...
/-- Goal `sum-two-squares-zmod-twentyfour-ne-nineteen`: over `ℤ`, `a ^ 2 + b ^ 2` is never `19` in `ZMod 24`,
by reducing the integer cast to `ZMod 24` and a finite case check. -/
theorem sum_two_squares_zmod_twentyfour_ne_nineteen (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 24) ≠ 19 := by
