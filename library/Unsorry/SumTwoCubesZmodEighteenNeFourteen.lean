...
/-- Goal `sum-two-cubes-zmod-eighteen-ne-fourteen`: over `ℤ`, `a ^ 3 + b ^ 3` is never `14` in `ZMod 18`,
by reducing the integer cast to `ZMod 18` and a finite case check. -/
theorem sum_two_cubes_zmod_eighteen_ne_fourteen (a b : ℤ) : (((a ^ 3 + b ^ 3 : ℤ)) : ZMod 18) ≠ 14 := by
