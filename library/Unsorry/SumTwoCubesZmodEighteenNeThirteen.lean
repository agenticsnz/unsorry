...
/-- Goal `sum-two-cubes-zmod-eighteen-ne-thirteen`: over `ℤ`, `a ^ 3 + b ^ 3` is never `13` in `ZMod 18`,
by reducing the integer cast to `ZMod 18` and a finite case check. -/
theorem sum_two_cubes_zmod_eighteen_ne_thirteen (a b : ℤ) : (((a ^ 3 + b ^ 3 : ℤ)) : ZMod 18) ≠ 13 := by
