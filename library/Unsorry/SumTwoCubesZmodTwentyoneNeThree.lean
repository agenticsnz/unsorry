...
/-- Goal `sum-two-cubes-zmod-twentyone-ne-three`: over `ℤ`, `a ^ 3 + b ^ 3` is never `3` in `ZMod 21`,
by reducing the integer cast to `ZMod 21` and a finite case check. -/
theorem sum_two_cubes_zmod_twentyone_ne_three (a b : ℤ) : (((a ^ 3 + b ^ 3 : ℤ)) : ZMod 21) ≠ 3 := by
