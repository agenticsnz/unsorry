...
/-- Goal `sum-two-cubes-zmod-twentyone-ne-seventeen`: over `ℤ`, `a ^ 3 + b ^ 3` is never `17` in `ZMod 21`,
by reducing the integer cast to `ZMod 21` and a finite case check. -/
theorem sum_two_cubes_zmod_twentyone_ne_seventeen (a b : ℤ) : (((a ^ 3 + b ^ 3 : ℤ)) : ZMod 21) ≠ 17 := by
