...
/-- Goal `sum-two-squares-zmod-sixteen-ne-eleven`: over `ℤ`, `a ^ 2 + b ^ 2` is never `11` in `ZMod 16`,
by reducing the integer cast to `ZMod 16` and a finite case check. -/
theorem sum_two_squares_zmod_sixteen_ne_eleven (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 16) ≠ 11 := by
