...
/-- Goal `sum-two-squares-zmod-twenty-ne-nineteen`: over `ℤ`, `a ^ 2 + b ^ 2` is never `19` in `ZMod 20`,
by reducing the integer cast to `ZMod 20` and a finite case check. -/
theorem sum_two_squares_zmod_twenty_ne_nineteen (a b : ℤ) : (((a ^ 2 + b ^ 2 : ℤ)) : ZMod 20) ≠ 19 := by
