...
- Ninth sourced target batch (ADR-012) — **compositeness via factorization** (difficulty 2–3): `pow-four-add-sq-add-one-factor` (n⁴+n²+1 = (n²+n+1)(n²−n+1) over ℤ — the Aurifeuillian-style factorization mathlib lacks), its capstone `pow-four-add-sq-add-one-not-prime` (n>1 → ¬Prime(n⁴+n²+1), `deps≜` the factorization leaf), and `one-add-four-b-fourth-not-prime` (b>1 → ¬Prime(1+4b⁴), the a=1 Sophie Germain corollary). Parallels the proved `not-prime-pow-four-add-four`. Two from-memory candidates dropped at the gate as already in mathlib (`sum_range_choose_sq` in Choose/Vandermonde, the Sophie Germain identity `pow_four_add_four_mul_pow_four'` in Algebra/Ring/Identities). Every case numerically verified; type-checks at rev c5ea00351c28; absence machine-checked.

- Eighth sourced target batch (ADR-012) — **triangular-number gems**, each compounding on the Gauss sum / Nicomachus (difficulty 2): `eight-triangular-add-one-eq-odd-sq` (8Tₙ+1 = (2n+1)², the triangular-number test), `consecutive-triangular-eq-square` (Tₙ+Tₙ₋₁ = n², Theon of Smyrna), and `cube-eq-triangular-sq-diff` (Tₙ₋₁²+n³ = Tₙ², the per-term form of Nicomachus, `deps≜ nicomachus-sum-cubes`). Tₙ stated as `∑ i ∈ range (n+1), i` so the prover must invoke the Gauss closed form. Numerically verified; type-checks at rev c5ea00351c28; absence machine-checked.

- Seventh sourced target batch (ADR-012) — **binomial moments**, compounding on the proved `sum-range-sq-mul-choose` (difficulty 2–3): `sum-range-cube-mul-choose` (8∑k³C(n,k) = n²(n+3)2ⁿ, the third moment, `deps≜ sum-range-sq-mul-choose`), `sum-range-fall-mul-choose` (4∑k(k−1)C(n,k) = n(n−1)2ⁿ, the second falling-factorial moment), and `sum-range-choose-mul-two-pow` (∑C(n,k)2ᵏ = 3ⁿ, the binomial theorem at x=2). mathlib has only the first moment `sum_range_mul_choose` and the unweighted row sum. Numerically verified; type-checks at rev c5ea00351c28; absence machine-checked. Board: 16 open / 61 proved / 78 total; gate-b DAG-valid.
