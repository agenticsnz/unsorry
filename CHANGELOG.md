...
- ADR-032/SPEC-032-A: a **proof-graph visualiser** (`tools/visualiser`, issue #371). V1 generates `docs/graph.md` — a GitHub-native Mermaid `flowchart` of the decomposition lineage (parent → sub-goal edges, status-coloured nodes, click-through to each Lean statement) plus a complete provenance table (every goal, status, difficulty, solver/model, proved-on date). The generator is **pure over the AISP records** (`goals/`, `decompositions/`, `library/index/`, `proof-runs/`), reusing `tools.gate_b.records` and `tools.leaderboard`, so `--check` is a deterministic drift guard and `--json` exposes the graph model for the Phase-2 interactive HTML and the leaderboard (#270) to share. Phase 2 (interactive `docs/graph.html`) and wiring `--check` into CI (touches `.github/`, ADR-019) are deferred to follow-up PRs.
