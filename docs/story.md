...
There was a subtler attack, too, from inside. Ocean — a founder out of New York, and soon the runaway leader of the board — quietly worked out that a whole class of the problems could be solved by a deterministic script, no model required, sliding right past the triviality checks. Two thousand of them landed overnight. *"A bit surprising to see in the morning,"* he admitted. It forced the uncomfortable, clarifying question the whole project turned on: *which proofs actually need an LLM, and which are just grinding?* The answer wasn't to ban the script. It was to **fold the deterministic solver in as an honest tool** for the easy targets — and reserve the expensive models for the hard ones. *"If we need to solve easy targets in service of a larger prize, we use the most efficient method."*

The bottleneck had flipped. It was no longer intelligence. It was coordination — the exact problem every successful organization eventually hits, except this one was four days old.

---

This is where the story becomes less about AI and more about a person staring at a dashboard at nine at night in the endless Finnish daylight.

Most people looked at unsorry and saw theorem proving. Perttu looked at it and saw infrastructure. While others competed on the leaderboard, he was reading runner metrics. While others celebrated new proofs, he was tracing memory. The agents, the model estimated, could produce **only ten to fifteen proofs an hour** — and the system saturated almost instantly, the verification choking long before the intelligence did.

The diagnoses were unglamorous and exactly right. The audit runners were pinned at a capacity of one; he moved the audit work onto plain Linux boxes. Duplicate proofs were flooding the queue — a rule against them had been *written* (in an architecture decision, ADR-017) but never *enforced*; he enforced it and cleared the backlog under forty. And then the embarrassingly simple one, the heart of it: the system was re-verifying proofs **that hadn't changed**, over and over, re-proving what it already knew. The fix — verify each proof once on the way in, then trust its immutability (ADR-048) — was the trust thesis turned into an optimization.

The numbers moved like weather clearing. The library build had been **233 seconds across 457 active modules**; by draining the verified set down to **about a dozen** and treating the rest as sealed archives, it dropped to seconds. The kernel replay had been quietly running a **16-gigabyte** runner out of memory; capping the work per pass fixed it. The axiom audit fell from **281 seconds to 141**. Per-proof verification time slid from a quarter-hour-plus toward **eight minutes**.
