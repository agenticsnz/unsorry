...
- **Phase 2 — offload.** Central CI re-checks high-risk proofs + a random sample; a peer quorum (e.g. 2-of-3, ≥1 independent of the calculator, **dispatcher vote unweighted**) gates normal proofs; honeypots + reputation gate validator standing; an open **challenge window** lets any node overturn a verdict with a reproducible counter-result (the kernel adjudicates).
- **Phase 3 — fallback.** Central CI is dispute / sampling / fallback only; the protected namespace still has a **final deterministic gate at promotion** (possibly the cheaper, portable `lean4export` cross-checker), retired *last*.
...
**Positive.** Verification becomes a credited, auditable, decentralisable role; the heavy central namespace.so cost is shifted to volunteers and scaled (ADR-049); produced proofs gain explicit, signed finality; an audited validator reputation table emerges as a by-product.

**Negative / accepted.** Validators need capable machines — the mathlib image is heavy (ADR-049; `lean4export` softens this). N-way validation multiplies compute (the win is cost-*shifting* + scaling + de-bottlenecking the central pool, not cheaper-per-check). Honeypots spend some real verification capacity to police honesty. The central gate is retired only slowly, and never fully at the promotion boundary.

**Deferred.** The implementation contract — roles wire format, attestation schema, honeypot rate, reputation math, challenge-window mechanics — lands in a SPEC at acceptance, phased like SPEC-049-A.
