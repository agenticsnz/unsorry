...
- **Phase 2 — pre-promotion offload (not a promotion gate).** A peer quorum (e.g. 2-of-3, ≥1 independent of the calculator, **dispatcher vote unweighted**) gates a **pre-promotion lane only** — prioritisation, credit, and pre-filtering (a candidate a quorum rejects never reaches the central gate, saving its compute). The mandatory central re-check stays **p = 1 at the promotion boundary** (ADR-049 / SPEC-049-A unchanged). Honeypots + reputation gate validator standing; an open **challenge window** lets any node overturn a *peer* verdict with a reproducible counter-result (the kernel adjudicates).
